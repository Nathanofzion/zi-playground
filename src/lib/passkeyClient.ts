...
const passkey = () => {
  return {
    ...
    getPasskeysInfo: async () => {
      // This will trigger the WebAuthn dialog showing available credentials
      try {
        const connectResult = await connectWithFactory(); // No keyId provided
        
        if (connectResult?.keyIdBase64 && connectResult?.contractId) {
          console.log('Found passkey credential in authenticator');
          console.log('KeyId:', connectResult.keyIdBase64);
          console.log('Contract:', connectResult.contractId);
          return true; // Passkey exists
        }
      } catch (error) {
        ...
        if (isNoCredentialError(error)) {
          console.log('No passkey credentials found in authenticator');
          return false; // No passkeys
        }
    ...
    getPublicKey: async () => {
      ...
      try {
        ...
        let storedKeyId;
        let storedContractId;

        if (activeWallet) {
          storedKeyId = activeWallet.keyId;
...
const contractId = activeWallet.contractId;
    ...
    createNewWallet: async (name: string) => {
      console.log('Creating new passkey wallet with name:', name);
      ...
      try {
        ...
        // Submit wallet creation transaction
        setPasskeyStatus('Deploying wallet...');
        ...
        setPasskeyStatus('Creation failed');
        throw error;
      }
    },
  };
